...
class ZipCode implements ZipCodeInterface
{
    ...
    {
        $this->buildXMLBody()->sendWebServiceRequest()
    ...
    protected function parseXMLFromResponse(): ZipCode
    {
        $parse = simplexml_load_string(
            str_replace([
                'soap:', 'ns2:',
            ], null, $this->response->getBody()->getContents())
        );
    ...
    {
        $this->body = trim(
            '<?xml version="1.0"?><soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:cli="http://cliente.bean.master.sigep.bsb.correios.com.br/"><soapenv:Header/><soapenv:Body><cli:consultaCEP><cep>'
            . $this->zipcode
    ...
    protected function fetchErrorMessage(): array
    {
        return [
            'error' => $this->messages(
                $this->parsedXML['Fault']['faultstring']
            ),
        ];
    }
