...
class Client
{
    ...
    public function freight(array $services, array $sizes): array
    {
        ...
        $freight = $this->freight->origin($this->zipcodes[0]->zipCode())
            ->destination($this->zipcodes[1]->zipCode())->services($services);
        foreach ($sizes as $size) {
            $freight->item(
                $size[$keys[0]], $size[$keys[1]], $size[$keys[2]],
                $size[$keys[3]], $size[$keys[4]]
            );
        }
        ...
        if (count($sizes)) {
            foreach ($sizes as $size) {
                ...
                for ($a = 0; $a < count($keys); $a++) {
                    $valid[] = array_key_exists($keys[$a], $size) ? 'verdadeiro'
                        : 'falso';
                }
